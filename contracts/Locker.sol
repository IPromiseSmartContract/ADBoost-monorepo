...
contract Locker {
    ...
    modifier onlyReceiver(uint256 lockUpId) {
        if (msg.sender != lockUps[lockUpId].receiver)
            revert LockUp__PermissionDenied();
        _;
    }
    ...
    function createLockUp(
        ...
        if (unlockTime <= block.timestamp)
            revert LockUp__InvalidParams("unlockTime");
    ...
    function unlock(uint256 lockUpId) external onlyReceiver(lockUpId) {
        ...
        if (lockUp.unlockTime > block.timestamp)
            revert LockUp__NotYetUnlocked();
        ...
        if (lockUp.isERC20) {
            ...
            IERC1155(lockUp.token).safeTransferFrom(
                address(this),
                lockUp.receiver,
                0,
                lockUp.amount,
    ...
    function getLockUpIdsByToken(
        ...
        if (start >= stop || stop - start > 10000)
            revert LockUp__InvalidPaginationParameters();
